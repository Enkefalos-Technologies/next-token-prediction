...
MODEL_NAME = "meta-llama/Llama-2-7b-chat-hf"  # "meta-llama/Llama-2-7b-chat-hf" or "gpt2"
...
def collect_topk_probs(top_tokens, top_probs, step, all_probs):
    """
    Collect the top-k tokens (string) and probabilities at one generation step.
    """
    all_probs.append((step, top_tokens, top_probs))

def display_topk_probs(top_tokens, top_probs, step):
    """
    Print the top-k tokens (string) and probabilities at one generation step.
    """
    print(f"\nStep {step + 1}:")
    for token, prob in zip(top_tokens, top_probs):
        clean_token = token.replace("\n", "\\n")
        print(f"  Token: '{clean_token}', Probability: {prob:.4f}")
...
def generate_next_tokens_huggingface(prompt, temperature):
    ...
    for step in range(MAX_NEW_TOKENS):
        ...
        collect_topk_probs(top_tokens, top_probs, step, all_probs)
        display_topk_probs(top_tokens, top_probs, step)
...
def plot_token_probabilities(all_probs, ax, title, is_first_column, is_last_row):
    ...
    if token_labels_str:
        ...
        ax.text(0.98, 0.98, token_text, fontsize=17, verticalalignment='top', horizontalalignment='right',
...
pdf_filename = "Users/pavan.yadav/exp/llama2_so_5march.pdf"
